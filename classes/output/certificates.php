...
class certificates implements renderable, templatable {
    ...
    public function export_for_template(renderer_base $output) {
        ...
        $certificates = new \theme_biossex\util\certificates($USER, $this->courseid);

        $issuedcertificates = $certificates->get_all_certificates();

        $title = get_string('certificatestitle', 'theme_biossex');
        $subtitle = get_string('subtitleallcertificates', 'theme_biossex');

        if ($this->courseid) {
            $subtitle = get_string('subtitlecoursecertificates', 'theme_biossex');
        }
