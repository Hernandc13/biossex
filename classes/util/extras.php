...
/**
 * Custom biossex extras functions
 *
...
class extras {
    ...
    public static function get_course_summary_image($course, $courselink) {
        ...
        if (empty($contentimage)) {
            $url = $CFG->wwwroot . "/theme/biossex/pix/default_course.jpg";
        ...
        if (!empty($CFG->messaging) && $USER->id != $user->id && has_capability('moodle/site:sendmessage', $context)) {
            ...
            $headerbuttons = [
                [
                    ...
                    'class' => 'btn btn-block btn-outline-#F380'
                ],
                [
                    'title' => get_string($contacttitle, 'theme_biossex'),
